        ...
        try:
            for _ in range(timeout):
                ...
                if container.status == 'running':
                    return f"Container {container_name} is running at {container.attrs['NetworkSettings']['IPAddress']} on port {container.attrs['NetworkSettings']['Ports']['5432/tcp'][0]['HostPort']}"
                time.sleep(1)
        except NotFound as e:
            return "Container {container_name} not found."
        except APIError as e:
            return "Failed to get container status: {str(e)}"
        except Exception as e:
            return f"[red]Failed to wait for container '{container_name}': {str(e)}[/red]"
