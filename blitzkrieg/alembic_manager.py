...
class AlembicManager:
    def __init__(self, workspace_path, db_manager, workspace_name: str = None):
    ...
    def check_alembic_env_path(self):
        try:
            self.console.print(f"Alembic env path: {str(self.alembic_env_path)} ")
            return True
    ...
    def check_alembic_ini_path(self):
        try:
            self.console.print(f"Alembic ini path: {str(self.alembic_ini_path)} ")
            return True
        except Exception as e:
...
if context.is_offline_mode():
    ...
    def run_migrations(self):
        self.console.display_step("Running Migrations", "Running initial migrations to set up workspace database...")
        try:
            subprocess.run(['alembic', 'upgrade', 'head'], cwd=self.workspace_path, check=True)
            return True
        except subprocess.CalledProcessError as e:
            self.console.display_notice(f"Failed to run migrations: {str(e)}")
            return False
